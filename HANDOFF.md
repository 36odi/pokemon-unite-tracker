...
> 最終更新: 2026-08-22 / アプリバージョン: v5.39 / 公開SW: v62 / ローカルSW: v62

## 0. 現在のスナップショット（2026-08-22 01:08 JST）

- v5.39 / SW v62で、ソルガレオをバランス型ロスターへ追加。対戦記録で「アイアンヘッド」「サイコショック」を選択・保存・編集復元できる。ラボは未対応。ソルガレオ対応として公開。
- 直前の公開版はv5.38 / SW v61（コミット `070c56a`）。2026-08-22 01:08 JSTに公開HTMLのv5.38と公開SW v61を実測した。
- 今回の検証結果は、自動テスト153/153 PASS（全98体のわざ・96×96画像を含む）、レシオ1,712/1,712一致、ステータス98/98体差分ゼロ、JavaScript構文20ファイル＋HTMLインライン1対象PASS、ラボ生成2ファイルのGit blob不変、`git diff --check`終了コード0。
- 未使用originの実ブラウザで、ソルガレオを検索・選択し、アイアンヘッド／サイコショックで1勝を保存。履歴表示と編集画面の復元、96×96アイコン、更新履歴v5.39、コンソールエラー0を確認した。ブラウザ操作面からService Worker/Cache Storageの内部状態は取得できなかったため、SW v62と旧キャッシュ削除は自動テストで確認した。
...
- 対戦用ロスターは98体（ソルガレオを含む）。ラボは98体（レシラム・ストライクを含む。ソルガレオは未対応）、スキル1,785行（数値照合対象1,712行）、メダル257枚、もちもの34個、メダル色11種。
...
- メインスプレッドシートの構造元sheet3/4は97体登録済み。full regen後にunite-db正本からレシラムを補完し、最終成果物を98体へ正本化する。
...
- **98匹** ポケモンのスキルデータ（計1,785行）
- **98匹** ポケモンのステータス（Lv1〜15）

---

### `sw.js`

Service Worker。ローカル・公開のキャッシュ名は `unite-tracker-v62`。`lab_data.js`、`js/constants.js`、`js/utils.js`、`js/lab-core.js`、PWA用192px・512pxアイコンを含むアプリシェルをキャッシュする。
