...
> 最終更新: 2026-08-05 / アプリバージョン: v5.37 / 公開SW: v59 / ローカルSW: v60

## 0. 現在のスナップショット（2026-08-05 17:48 JST）

- ローカルv5.37 / SW v60で、8月7日参戦予定のレシラムを対戦用ロスターへ追加。アタック型、図鑑ID 643、わざ1「りゅうのまい」／わざ2「あおいほのお」を選択・保存できる。レシオ・ステータス未判明のためラボは未対応、未push。
- Phase 4はPR #2で `main` へマージ済み（マージコミット `8bfe5e4`）。PWA設定修正、シリーズ全件取得のページング、ラボ計算依存の明示化、シリーズ分析概要の純粋関数化を、4コミットへ分離して実装した。
- 公開は v5.36 / SW v59。GitHub Pages build `1128923769` が `8bfe5e4` を対象に完了し、公開URLで新Service Workerへの更新後に実ブラウザ検証済み。
...
- 検証済み: `node tests/logic.test.js` 131/131 PASS。レシオ1701/1701一致、ステータス97/97体差分ゼロ、レシラムのロスター・図鑑ID・わざ・画像検証PASS、変更対象JS構文・`git diff --check`終了コード0。
...
- 対戦用ロスターは97体（レシラムを含む）。ラボは97体（レシラム未対応・ストライクを含む）、スキル1,773行、メダル257枚、もちもの34個、メダル色11種。
...
Service Worker。ローカルのキャッシュ名 `unite-tracker-v60`（公開はv59）。`lab_data.js`、`js/constants.js`、`js/utils.js`、`js/lab-core.js`、PWA用192px・512pxアイコンを含むアプリシェルをキャッシュする。
