...
## 0. 現在のスナップショット（2026-07-24 22:59 JST）
...
- フル再生成に必要な `%TEMP%/sheet1_medal.csv` ～ `sheet4_status.csv` は現在ローカルに無い。メインスプレッドシートのsheet3/4は92体のままで、現行ラボ97体のうち **イベルタル／ウェーニバル／メガニウム／ラウドボーン／パルキア** の5体が未登録。このままフル再生成すると5体が消える可能性がある。
