...
> 最終更新: 2026-07-17 / アプリバージョン: v5.33 / 公開SW: v54 / ローカルSW: v54

## 0. 現在のスナップショット（2026-07-17 15:57 JST）

- 公開 `main` は v5.33 / SW v54。パルキアを対戦用ロスターへ追加（バランス型、図鑑ID 484、はどうだん／ドラゴンクロー）。ラボのステータス・わざレシオは未追加。
...
- 検証済み: `node tests/logic.test.js` 85/85 PASS、レシオ1689/1689一致、ステータス96/96体差分ゼロ、変換器の構造ドリフト・オラクル不一致ともに0。
- 対戦用ロスターは96体。ラボは96体（ストライクを含む・パルキア未対応）、スキル1,761行、メダル257枚、もちもの34個、メダル色11種。
- `gen_lab_data.js` は `lab_data_generated.js` と本番用 `lab_data.js` の両方へ直接出力し、その後 unite-db 正本化を自動実行する。
- フル再生成に必要な `%TEMP%/sheet1_medal.csv` ～ `sheet4_status.csv` は現在ローカルに無い。2026-07-17にメインスプレッドシートを直接確認した結果、sheet3/4はいずれも92体で、現行ラボ96体のうち **イベルタル／ウェーニバル／メガニウム／ラウドボーン** の4体が未登録。このままフル再生成すると4体が消える可能性がある。
...
- [ ] **メインスプレッドシート sheet3/4 へ不足4体を登録**
  - 2026-07-17確認: sheet3/4はいずれも92体で、イベルタル／ウェーニバル／メガニウム／ラウドボーンが未登録。
  - 現行 `lab_data.js` は96体。構造元に4体を追加するまで `gen_lab_data.js` のフル再生成は禁止。
