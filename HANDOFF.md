...
> 最終更新: 2026-08-21 / アプリバージョン: v5.38 / 公開SW: v60 / ローカルSW: v61

## 0. 現在のスナップショット（2026-08-21 23:03 JST）

- ローカルv5.38 / SW v61で、レシラムをラボへ追加。Lv1〜15ステータス、通常・強化攻撃、りゅうのいぶき、あおいほのお（やけど5回）、りゅうのまい（シールド）、烈火招雷（ほのお／いかずち）を計算できる。未push。
- 公開はv5.37 / SW v60（コミット `4247a80`）。レシラムの対戦記録対応まで公開済みで、今回のラボ対応はローカルのみ。
...
- 今回の検証結果は、自動テスト148/148 PASS、レシオ1,712/1,712一致、ステータス98/98体差分ゼロ、再生成のハッシュ不変・生成2ファイル同一、JavaScript構文21対象すべてPASS。
...
- 対戦用ロスターは97体（レシラムを含む）。ラボは98体（レシラム・ストライクを含む）、スキル1,785行（数値照合対象1,712行）、メダル257枚、もちもの34個、メダル色11種。
...
Service Worker。ローカルのキャッシュ名 `unite-tracker-v61`（公開はv60）。`lab_data.js`、`js/constants.js`、`js/utils.js`、`js/lab-core.js`、PWA用192px・512pxアイコンを含むアプリシェルをキャッシュする。
